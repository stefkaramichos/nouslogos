...
@section('title', 'Επεξεργασία Αρχείου')
...
@section('content')
    ...
    <div class="card">
        <div class="card-header">Επεξεργασία αρχείου #{{ $document->id }}</div>
        <div class="card-body">
            <form method="POST" action="{{ route('documents.update', $document) }}" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <select name="customer_id" class="form-select" required>
                        ...
                        @foreach(($customers ?? []) as $c)
                            <option value="{{ $c->id }}" @selected(old('customer_id', $document->customer_id) == $c->id)>
                ...
                <div class="mb-3">
                    ...
                    <select name="visible_professional_id" class="form-select">
                        ...
                        @foreach(($professionals ?? []) as $p)
                            <option value="{{ $p->id }}" @selected(old('visible_professional_id', $document->visible_professional_id) == $p->id)>
                ...
                <div class="mb-3">
                    <label class="form-label">Τρέχον αρχείο</label>
                    <div class="form-control-plaintext">
                        <strong>{{ $document->original_name }}</strong>
                        <span class="text-muted">({{ $document->mime_type ?? '—' }})</span>
                    </div>
                </div>

                <div class="mb-3">
                    <label class="form-label">Σημείωση</label>
                    <textarea name="note" class="form-control" rows="4">{{ old('note', $document->note) }}</textarea>
                    @error('note')
                        <div class="text-danger small mt-1">{{ $message }}</div>
                    @enderror
                </div>
